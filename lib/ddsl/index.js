...
DDSL.prototype.exportApply = function exportApply(exports) {
  ...
  this.oninit.push(function(exports) {
    exports.BEMContext.prototype.settings = exports.settings;
...
DDSL.prototype.runOne = function runOne(json) {
  if(json.block && this.context.settings && this.context.settings.constructors) {
    var constructors = this.context.settings.constructors;
    // we must check replacement
    var exists = pascalCase(json.block);
    if(constructors[exists]) {
      // FIXME: strong dependency on React
      var args = [exists, p2m.reactify(json)].concat(json.content);
